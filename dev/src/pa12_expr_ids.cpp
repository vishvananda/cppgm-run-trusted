...
} vector<pa11::TemplateInstanceArgument> pack; for (size_t i = 0; i < argument.pack.size(); ++i) pack.push_back(id_template_instance_argument(argument.pack[i]));
...
TypePtr base = record.get() != NULL && record->base.get() != NULL ? pa11::strip_cv(record->base) : TypePtr(); if (base.get() != NULL && base->kind == pa11::TypeKind::Record &&
base->scope != NULL) return find_variable_templates_in_scope_tree(base->scope, name, variable_templates,
out, seen); return false; }
...
Expr Parser::make_template_substitution_id_expr(const QualifiedName& name) { if (name.qualified || name.has_template_arguments) return Expr(); vector<Binding*> parameter_pack; if (find_function_parameter_pack_substitution(name.name, parameter_pack)) { Expr out; out.valid = true; out.pack_expansion = true; out.type = parameter_pack.empty() ? pa11::make_fundamental(FT_INT) : parameter_pack[0]->type; out.category = ValueCategory::LValue; out.node = Node("pack-expression " + name.name); for (size_t i = 0; i < parameter_pack.size(); ++i) { Expr elem; elem.valid = true; elem.binding = parameter_pack[i]; elem.type = parameter_pack[i]->type; elem.category = ValueCategory::LValue;
elem.node = Node("id-expression lvalue " + pa11::describe_type(elem.type) + " " + parameter_pack[i]->name); elem.node.binding = parameter_pack[i]; annotate_expr_node(elem); out.pack.push_back(elem); add_child(out.node, elem.node); } annotate_expr_node(out); return out; } TemplateArgument value_arg; if (find_template_value_substitution(name.name, value_arg) && value_arg.kind == TemplateArgumentKind::Pack) { Expr out; out.valid = true; out.pack_expansion = true; out.type = value_arg.pack.empty() || value_arg.pack[0].type.get() == NULL ? pa11::make_fundamental(FT_INT) : value_arg.pack[0].type; out.category = ValueCategory::PRValue; out.dependent_value_name = name.name; out.node = Node("pack-expression " + name.name); for (size_t i = 0; i < value_arg.pack.size(); ++i) { if (value_arg.pack[i].kind != TemplateArgumentKind::Value)
throw runtime_error("value pack required"); Expr elem; elem.valid = true; elem.type = value_arg.pack[i].type.get() != NULL ? value_arg.pack[i].type : pa11::make_fundamental(FT_INT); elem.category = ValueCategory::PRValue; elem.constant_expression = !value_arg.pack[i].dependent; elem.has_constant_value = !value_arg.pack[i].dependent; elem.constant_value = value_arg.pack[i].value; elem.dependent_value_name = value_arg.pack[i].value_name.empty() ? name.name : value_arg.pack[i].value_name; elem.dependent_value_owner_template_name = value_arg.pack[i].value_owner_template_name; elem.dependent_value_member_name = value_arg.pack[i].value_member_name; elem.dependent_value_negated = value_arg.pack[i].value_negated; elem.dependent_value_owner_template_arguments =
value_arg.pack[i].value_owner_template_arguments; if (value_arg.pack[i].dependent) elem.node = Node("id-expression prvalue " + pa11::describe_type(elem.type) + " " + name.name); else { elem.node = Node("literal prvalue " + pa11::describe_type(elem.type) + " " + to_string(elem.constant_value)); elem.node.token_text = to_string(elem.constant_value); } annotate_expr_node(elem); out.pack.push_back(elem); add_child(out.node, elem.node); } annotate_expr_node(out); return out; } if (find_template_value_substitution(name.name, value_arg) && value_arg.kind == TemplateArgumentKind::Value) { Expr out; out.valid = true; out.type = value_arg.type.get() != NULL ? value_arg.type : pa11::make_fundamental(FT_INT); out.category = ValueCategory::PRValue; out.constant_expression = true; out.has_constant_value =
!value_arg.dependent && value_arg.value_binding == NULL; out.constant_value = value_arg.value; out.null_pointer_constant = out.constant_value == 0; if (value_arg.value_binding != NULL) { if (unevaluated_expression_depth_ == 0 && value_arg.value_binding->kind == BindingKind::Function) { parse_pending_function_body(value_arg.value_binding); parse_pending_member_body(value_arg.value_binding); } out.type = value_arg.value_binding->type; out.category = ValueCategory::LValue; out.constant_value = 0; out.null_pointer_constant = false; out.node = Node("id-expression lvalue " + pa11::describe_type(out.type) + " " + qualified_decl_name(value_arg.value_binding)); } else { if (value_arg.dependent) { string value_name = value_arg.value_name.empty() ? name.name : value_arg.value_name; out.dependent_value_name = value_name;
out.dependent_value_owner_template_name = value_arg.value_owner_template_name; out.dependent_value_member_name = value_arg.value_member_name; out.dependent_value_negated = value_arg.value_negated; out.dependent_value_owner_template_arguments = value_arg.value_owner_template_arguments; out.node = Node("id-expression prvalue " + pa11::describe_type(out.type) + " " + value_name); } else { out.node = Node("literal prvalue " + pa11::describe_type(out.type) + " " + to_string(out.constant_value)); out.node.token_text = to_string(out.constant_value); } } annotate_expr_node(out); if (value_arg.value_binding != NULL) out.node.binding = value_arg.value_binding; return out; } return Expr(); }
