...
if (friend_class_scope != NULL && qname.qualifier == NULL) target = nearest_namespace_scope(friend_class_scope); TypePtr type = apply_declarator(declarator, base); if (specs.typedef_decl) {
...
!pa11::is_reference_type(type) && type->kind != pa11::TypeKind::Function) type = pa11::make_cv(type, pa11::CV_CONST); if (init != NULL && init->braced_init_list && type->kind == pa11::TypeKind::Array &&
type->unknown_bound) type = pa11::make_array(type->base, false, init->node.children.size()); if (init != NULL && type->kind == pa11::TypeKind::Array && type->unknown_bound) { uint64_t elements = 0;
if (string_literal_initializes_array(type, *init, &elements)) type = pa11::make_array(type->base, false, elements); } bool existing_static_member_function = false; if (target->kind == ScopeKind::Class &&
