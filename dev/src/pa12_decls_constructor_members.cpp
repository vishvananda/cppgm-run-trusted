                                           ...
                                           bool constexpr_ctor)
{
	...
	for (size_t i = 0; i < signature_parameter_indices.size(); ++i)
		ctor_names.push_back(
			parameters[signature_parameter_indices[i]].name);
	...
	if (consume(OP_ASS))
	{
		if (consume(KW_DEFAULT))
		{
			...
			{
				ParameterInfo& parameter =
					parameters[signature_parameter_indices[0]];
				if (pa11::is_reference_type(parameter.type) &&
			    pa11::same_type(pa11::strip_cv(parameter.type->base),
