...
struct DeclSpecs
{
	...
	DeclSpecs() : typedef_decl(false), cv(CV_NONE) {}
};
...
private:
	...
	bool lookahead_identifier(size_t offset) const
	{
		size_t index = pos_ + offset;
		return index < tokens_.size() &&
	...
	void parse_declaration()
	{
		if (consume(OP_SEMICOLON))
			return;
		if (at(KW_INLINE) || at(KW_NAMESPACE))
		{
	...
	void parse_namespace_or_alias()
	{
		...
		if (!inline_ns && at_identifier() && lookahead(OP_ASS, 1))
		{
			...
			expect(OP_SEMICOLON);
			add_namespace_alias(tu_, current_namespace(), alias, target);
			return;
		}
		string name;
		bool named = false;
	...
	void parse_using_declaration_family()
	{
		...
		if (at_identifier() && lookahead(OP_ASS, 1))
		{
			...
			expect(OP_SEMICOLON);
			add_type_alias(tu_, current_namespace(), name, type);
			return;
		}
	...
	void parse_simple_declaration()
	{
		...
		for (;;)
		{
			...
			if (specs.typedef_decl)
				add_type_alias(tu_, target, qname.name, type);
			else if (type->kind == TypeKind::Function)
				add_function(tu_, target, qname.name, type);
			else
				add_variable(tu_, target, qname.name, type);
			if (!consume(OP_COMMA))
				break;
		}
		expect(OP_SEMICOLON);
	...
	void parse_noptr_declarator_root(Declarator& declarator)
	{
		...
		declarator.name = parse_id_expression();
		declarator.has_name = true;
	}

	QualifiedName parse_id_expression()
	{
		QualifiedName name;
		if (starts_nested_name_specifier())
	...
	Suffix parse_array_suffix()
	{
		...
		suffix.bound = parse_array_bound();
		expect(OP_RSQUARE);
		return suffix;
	}

	unsigned long long parse_array_bound()
	{
		if (!at_literal())
			throw runtime_error("expected array bound");
