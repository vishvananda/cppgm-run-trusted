...
bool top_destructor_body = top_function_body && is_class_destructor_binding(fn_.binding);
bool complete_destructor_epilogue = top_destructor_body &&
	(fn_.binding->is_generated_default_destructor || fn_.binding->is_defaulted);
...
constructor_unwind_actions_.push_back(&node.children[i]); } if (complete_destructor_epilogue || (top_destructor_body && !destructor_has_fini_actions)) maybe_lower_destructor_epilogue(destructor_has_fini_actions);
...
{ TypePtr record = class_record_for_member(fn_.binding); if (record.get() != NULL) lower_vptr_store(record);
	} bool scope_affects_unwind = cleanup_scope_affects_unwind(cleanups_.back());
...
if (scope_affects_unwind) clear_active_unwind_dispatch();
...
throw runtime_error("break outside loop or switch"); emit_control_transfer_cleanups(break_cleanup_depths_.back()); terminate("jump ^" + break_targets_.back()); } else if (starts_with(node.line, "continue-statement"))
	{ if (continue_targets_.empty()) throw runtime_error("continue outside loop"); emit_control_transfer_cleanups(continue_cleanup_depths_.back()); terminate("jump ^" + continue_targets_.back());
...
TypePtr child_object = pa11::strip_cv(object_type(expr.children[0].type)); if (expr.children[0].category == ValueCategory::LValue && child_object->kind == TypeKind::Record) ensure_pointer(emit_lvalue_addr(expr.children[0]));
	...
	if (starts_with(expr.line, "call-expression") &&
	    (is_reference(expr.type) ||
	     ...
	     expr.category == ValueCategory::XValue)) { TypePtr object = pa11::strip_cv(object_type(expr.type)); if (object->kind == TypeKind::Record ||
...
void FunctionLowerer::lower_if(const Node& node)
{
	...
	const Node& cond = node.children[0].children[0];
		...
		string incoming_unwind_dispatch = active_unwind_dispatch_;
		string merged_unwind_dispatch = incoming_unwind_dispatch;
		size_t incoming_unwind_depth = active_unwind_cleanup_depth_;
		size_t merged_unwind_depth = incoming_unwind_depth;

		start_block(then_block);
		active_unwind_dispatch_ = incoming_unwind_dispatch;
		active_unwind_cleanup_depth_ = incoming_unwind_depth;
		cleanups_.push_back(vector<Cleanup>());
		lower_stmt(node.children[1].children[0]);
		bool then_done = current_ == NULL || current_->terminated;
		if (cleanups_.back().empty() &&
		    !active_unwind_dispatch_.empty())
		{
			merged_unwind_dispatch = active_unwind_dispatch_;
			merged_unwind_depth = active_unwind_cleanup_depth_;
		}
		if (current_ != NULL && !current_->terminated) {
			emit_scope_cleanups(cleanups_.back());
		}
		cleanups_.pop_back();
		active_unwind_dispatch_ = merged_unwind_dispatch;
		active_unwind_cleanup_depth_ = merged_unwind_depth;
	if (current_ != NULL && !current_->terminated) {
		...
		bool else_done = current_ == NULL || current_->terminated;
		if (cleanups_.back().empty() &&
		    !active_unwind_dispatch_.empty())
		{
			merged_unwind_dispatch = active_unwind_dispatch_;
...
continue_cleanup_depths_.push_back(cleanups_.size()); break_targets_.push_back(end_block); break_cleanup_depths_.push_back(cleanups_.size()); branch_on(node.children[0].children[0], body_block, end_block); start_block(body_block); lower_stmt(node.children[1]);
if (!current_->terminated) terminate("jump ^" + cond_block); break_cleanup_depths_.pop_back(); break_targets_.pop_back(); continue_cleanup_depths_.pop_back(); continue_targets_.pop_back();
...
start_block(body_block); continue_targets_.push_back(cond_block); continue_cleanup_depths_.push_back(cleanups_.size()); break_targets_.push_back(end_block); break_cleanup_depths_.push_back(cleanups_.size()); lower_stmt(node.children[0]);
...
break_cleanup_depths_.pop_back(); break_targets_.pop_back(); continue_cleanup_depths_.pop_back(); continue_targets_.pop_back(); start_block(end_block); }
...
start_block(cond_block); continue_targets_.push_back(iter_block); continue_cleanup_depths_.push_back(cleanups_.size()); break_targets_.push_back(end_block); break_cleanup_depths_.push_back(cleanups_.size()); if (node.children.size() > 1 && starts_with(node.children[1].line, "condition"))
...
terminate("jump ^" + cond_block); break_cleanup_depths_.pop_back(); break_targets_.pop_back(); continue_cleanup_depths_.pop_back(); continue_targets_.pop_back(); start_block(end_block);
if (init_cleanup_scope) { bool scope_affects_unwind = cleanup_scope_affects_unwind(cleanups_.back()); emit_scope_cleanups(cleanups_.back()); if (scope_affects_unwind) clear_active_unwind_dispatch(); cleanups_.pop_back();
...
continue_cleanup_depths_.push_back(cleanups_.size()); break_targets_.push_back(end_block); break_cleanup_depths_.push_back(cleanups_.size()); branch_on(condition, body_block, end_block); start_block(body_block); TypePtr loop_type = loop_var.binding->type;
...
lower_discarded_expr(increment); terminate("jump ^" + cond_block); break_cleanup_depths_.pop_back(); break_targets_.pop_back(); continue_cleanup_depths_.pop_back(); continue_targets_.pop_back();
...
string end_block = fresh_block("for_end"); terminate("jump ^" + cond_block); start_block(cond_block); continue_targets_.push_back(iter_block);
continue_cleanup_depths_.push_back(cleanups_.size()); break_targets_.push_back(end_block); break_cleanup_depths_.push_back(cleanups_.size()); string idx_loaded = fresh_temp(); instr(idx_loaded + " = load i32 $" + slot_for(idx_var.binding)); string cmp;
...
terminate("jump ^" + cond_block); break_cleanup_depths_.pop_back(); break_targets_.pop_back(); continue_cleanup_depths_.pop_back(); continue_targets_.pop_back(); start_block(end_block);
	...
	ActiveCatchContext ctx;
		...
		ctx.entry = entry;
		ctx.selector = 1;
		ctx.cleanup_depth = cleanups_.size();
	active_catches_.push_back(ctx);
	instr("eh_try ^" + dispatch);
	++eh_try_depth_;
	cleanups_.push_back(vector<Cleanup>());
	cleanups_.back().push_back(Cleanup("eh_end"));
	string saved_active_unwind_dispatch = active_unwind_dispatch_;
	size_t saved_active_unwind_depth = active_unwind_cleanup_depth_;
	active_unwind_dispatch_.clear();
	active_unwind_cleanup_depth_ = 0;
	lower_stmt(protected_body);
	active_unwind_dispatch_ = saved_active_unwind_dispatch;
	active_unwind_cleanup_depth_ = saved_active_unwind_depth;
	cleanups_.pop_back();
	--eh_try_depth_;
	...
	string try_end = fresh_block("try_end");
		for (size_t i = 0; i < catches.size(); ++i) {
			...
			catches[i].ctx.cleanup_depth = cleanups_.size();
...
break_targets_.push_back(end); break_cleanup_depths_.push_back(cleanups_.size()); lower_stmt(node.children[1]); if (!current_->terminated) terminate("jump ^" + end);
break_cleanup_depths_.pop_back(); break_targets_.pop_back(); for (size_t i = 0; i < installed.size(); ++i) switch_labels_.erase(installed[i]); start_block(end);
}
}  // namespace internal
}  // namespace pa14
